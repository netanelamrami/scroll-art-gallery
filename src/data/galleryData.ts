...
export const generateGalleryImages = async (count: number = 2000): Promise<GalleryImage[]> => {
  const data = await apiService.getEventImagesFullData();
  ...
  return Array.from({ length: Math.min(count, imagesFromServer.length) }, (_, index) => {
    ...
    return {
      id: `${imageData.name}`,
      src: imageData.smallUrl, // או imageData.src לפי מה שמגיע מהשרת
